...
st.title('Visualising ECGs from the broad QRS DDRTree')
...
def load_data():
    ...
    return {
        ...
        'decoded_ecgs': decoded_ecgs_array_12L,
        'decoded_stds': decoded_stds_array_12L,
        'tree_data': broadqrs_ddrtree
    }

data = load_data()

# Create DDRTree figure
def create_ddrtree_fig():
    fig = go.Figure()
    fig.add_trace(go.Scattergl(
        ...
        marker=dict(
            size=7, 
            color=data['tree_data']['color_new'], 
            opacity=0.7, 
            line=dict(width=1, color='black')
        ),
        name='Scatter Points',
        hoverinfo='x+y+text',
        text=data['tree_data']['tips_type_nice']
    ))
    fig.update_layout(
...
# Create empty ECG figure
def create_empty_ecg_fig():
    fig = go.Figure()
    fig.update_layout(
        annotations=[dict(
            x=0.5, y=0.5,
            text="Click on a point on the tree to see the reconstructed ECG.",
            showarrow=False,
...
def create_ecg_fig(point_index):
    tips_type = data['tree_data'].iloc[point_index]['tips_type_mapped']
    ecg_data = data['decoded_ecgs'][tips_type]
    ...
    fig = plot_ecg_plotly(
        ...
        subplots=True, 
        figsize=(1000, 1000), 
        std=data['decoded_stds'][tips_type]
    )
    
    phenogroup = data['tree_data'].iloc[point_index]['tips_type_nice']
    point_color_name = data['tree_data'].iloc[point_index]['color_new']
    rgb_color = color_map1.get(point_color_name, (0,0,0))
...
if 'ddrtree_fig' not in st.session_state:
    st.session_state.ddrtree_fig = create_ddrtree_fig()
if 'selected_point' not in st.session_state:
    st.session_state.selected_point = None

# Create columns (always show both)
col1, col2 = st.columns(2)

# Left Column - DDRTree (Static)
with col1:
    ddrtree_fig = create_ddrtree_fig()
    selected = st.plotly_chart(
        ddrtree_fig,
        on_select="rerun",
        key="ddrtree_selection",
    ...
    try:
        st.session_state.selected_point = selected["selection"]["points"][0]["point_index"]
    except (TypeError, KeyError, IndexError):
        pass 

# Right Column - ECG (Dynamic)
with col2:
    if st.session_state.selected_point is not None:
        ecg_fig = create_ecg_fig(st.session_state.selected_point)
        st.plotly_chart(ecg_fig, use_container_width=True)
    else:
        empty_fig = create_empty_ecg_fig()
